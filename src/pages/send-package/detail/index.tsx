...
const DetailPage = () => {
  ...
  const downloadPdf = async () => {
    if (!id) return; // Memastikan ID ada sebelum memproses

    try {
      setDownloadingPdf(true); // Memulai status loading
      toast.success("PDF akan diunduh"); // Memberi feedback awal ke user

      // Menjalankan mutasi download dan menunggu hingga selesai
      await downloadPdfMutation.mutateAsync(parseInt(id));

      toast.success("PDF berhasil diunduh"); // Feedback sukses
    } catch (error) {
      // Logging dan feedback jika terjadi kesalahan
      console.error("Failed to download PDF:", error);
      toast.error("Gagal mengunduh PDF");
    } finally {
      setDownloadingPdf(false); // Menghentikan status loading
    }
  };
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-1 min-w-0 flex-1">
                      <h2 className="font-semibold truncate">
                        {detail?.user?.name || "Tidak tersedia"}
                      </h2>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-1 min-w-0 flex-1">
                      <h2 className="font-semibold break-all">
                        {detail?.user?.phone_number || "Tidak tersedia"}
                      </h2>
              ...
              <Card className="rounded-2xl">
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-1 min-w-0 flex-1">
                      <h2 className="font-semibold truncate">
                        {detail?.recipient_name || "Tidak tersedia"}
                      </h2>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-1 min-w-0 flex-1">
                      <h2 className="font-semibold break-all">
                        {detail?.recipient_phone || "Tidak tersedia"}
                      </h2>
                      <p className="text-sm text-secondary">Nomor Telepon</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="font-semibold capitalize truncate">
                          {detail?.package_type?.toLowerCase() ||
                            "Tidak tersedia"}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="font-semibold capitalize truncate">
                          {detail?.delivery_type?.toLowerCase() || "Regular"}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="font-semibold break-all">
                          {shipment.tracking_number || "Tidak tersedia"}
                        </p>
                      </div>
                    </div>
                  </div>
                  {shipment.delivery_status && (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <Badge
                          variant={
                            getStatusBadgeVariant(shipment.delivery_status) as
                              | "default"
                              | "outline"
                              | "darkGreen"
                              | "destructive"
                          ...
                          className="mt-1"
                        >
                          {getStatusLabel(shipment.delivery_status)}
                        </Badge>
                      </div>
                    </div>
                  )}
                      ...
                      .map((history, index) => (
                        <div key={history.id} className="flex gap-4">
                          <div className="flex flex-col items-center flex-shrink-0">
                            ...
                            {index <
                              shipment.shipment_histories!.length - 1 && (
                  ...
                  {detail ? (
                    <>
                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <span className="text-lg font-semibold flex-shrink-0">
                          {formatPrice(detail.weight_price)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex gap-3 min-w-0">
                        ...
                        <span className="text-lg font-semibold flex-shrink-0">
                          {formatPrice(detail.distance_price)}
                        </span>
                      </div>
